...
const MyCargoOffersScreen = ({ navigation, route }) => {
  ...
  const farmerId_param = route?.params?.farmerId;

  useFocusEffect(
    React.useCallback(() => {
      if (farmerId_param) {
        setFarmerId(farmerId_param);
        loadOffers(farmerId_param);
      }
    }, [farmerId_param])
  );
    ...
    <Card key={offer.offer_id} style={styles.offerCard}>
      ...
      <View style={styles.capacitySection}>
        <View style={styles.capacityItem}>
          ...
          <Text style={styles.capacityValue}>{offer.cargo_volume_total?.toFixed(2)}m³</Text>
        </View>
        <View style={styles.capacityItem}>
          <Text style={styles.capacityLabel}>Available</Text>
          <Text style={styles.capacityValue}>{offer.cargo_volume_available?.toFixed(2)}m³</Text>
        </View>
        <View style={styles.capacityItem}>
          <Text style={styles.capacityLabel}>Booked</Text>
          <Text style={styles.capacityValue}>
            {((offer.cargo_volume_total || 0) - (offer.cargo_volume_available || 0)).toFixed(2)}m³
          </Text>
        </View>
      </View>
      ...
      <View style={styles.windowSection}>
        ...
        <Text style={styles.windowTime}>
          {new Date(offer.delivery_window_start).toLocaleDateString()} • {new Date(offer.delivery_window_start).toLocaleTimeString([], {
            hour: '2-digit',
            minute: '2-digit'
